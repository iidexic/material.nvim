...
--- supposed to extend colors
---@param colors table[table]
function M.make_edit_groups(colors)
	local eg = {}
	eg.darker = {
		editor = {
			bg = "#212121",
...
function M.load_edit_group(colors, id)
	if not colors.editor then
		colors.editor = {}
	end
	if not colors.lsp then
		colors.lsp = {}
	end
	if not colors.backgrounds then
		colors.backgrounds = {}
	end
	if not colors.syntax then
		colors.syntax = {}
	end

	if M.edit_groups[id] then
		return vim.tbl_deep_extend("force", colors, M.edit_groups[id])
	end
	return vim.tbl_deep_extend("force", colors, M.edit_groups.darker) --default
end
return M
